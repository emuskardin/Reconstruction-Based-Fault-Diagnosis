...
def to_hidden_layers(hidden_dimension, dropout):
    layers = []
    for i in range(len(hidden_dimension) - 1):
        layers.append(nn.Linear(hidden_dimension[i], hidden_dimension[i + 1]))
        layers.append(nn.ReLU())
        if dropout > 0.0:
            layers.append(nn.Dropout(dropout))  # Apply dropout if set
...
class AutoEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dimension, dropout=0, device=None):
        ...
        # encoder
        encoder_layers = to_hidden_layers(hidden_dimension, dropout)
        ...
        # decoder
        decoder_layers = to_hidden_layers(hidden_dimension, dropout)
...
def load_autoencoder(path_to_folder, epoch=None):
    metadata = json.load(open(f'{path_to_folder}/metadata.json'))
    model_weights = f'{path_to_folder}/model.pt' if epoch is None else f'{path_to_folder}/model_epoch_{epoch}.pt'
...
def train_autoencoder(model, train_loader, num_epochs, model_name=None, learning_rate=0.001,
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if save_every and epoch % save_every == 0:
            torch.save(model.state_dict(), f'{save_path}/{model_name}/model_epoch_{epoch}.pt')
